estimate_bp_disc_optim = function(data, bp=3) {
  ...
  create_data_est_model = function(bp_vec, data = dat) {
    ...
    mod = tryCatch(MASS::glm.nb(backpro~., data = dat_mod), error = NULL)
  ...
  # Re-estimate model
  mod = MASS::glm.nb(backpro~., data = dat_mod)
...
estimate_bp_segmented = function(data, bp = 3, start_bp = c(18,23,54), 
  ...
  dat = data %>% mutate(t=1:n())
  mod1 = MASS::glm.nb(backpro~t, data=dat)
  ...
  bp_plot = ggplot() +
    ...
    ggsave("D:/Bavaria/before_may_nb/disease_onset_ba_nb.png", width = 9, height = 5)
  ...
  bp_plot_infection = ggplot() +
    ...
    theme(
      ...
      legend.title =element_text(size = rel(1.5)),
      legend.position = "bottom") +
    ggsave("D:/Bavaria/before_may_nb/disease_onset_bav_shifted_nb.png", width = 9, height = 5)
